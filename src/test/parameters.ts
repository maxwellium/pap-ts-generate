...
//TODO: mutate? range.ZMVB = 0;

/**  In JRE4 enthaltene Entschädigungen nach § 24 Nummer 1 EStG in Cent  */
//TODO: mutate? range.JRE4ENT = BigDecimal.ZERO();

/**  In SONSTB enthaltene Entschädigungen nach § 24 Nummer 1 EStG in Cent  */
//TODO: mutate? range.SONSTENT = BigDecimal.ZERO();


interface Outputs { [ k: string ]: number };
interface Inputs { [ k: string ]: number };
   ...
   for ( let parameter in range ) {
      if ( 'object' === typeof range[ parameter ] ) {
         if ( range[ parameter ].hasOwnProperty( 'length' ) ) {
            ...
            let min = range[ parameter ].min || 0,
               span = ( range[ parameter ].max || 0 ) - min;
